...
export function Input({ leadingIcon, trailingIcon, invalid, size='large', style, ...rest }) {
  const [f,setF]=React.useState(false);
  const h = size==='small'?32:40;
  const bd = invalid ? t.danger : f ? t.blue500 : t.stroke;
  return React.createElement('div',{style:{display:'flex',alignItems:'center',gap:8,height:h,
    padding:'0 12px',background:t.surface,border:'1px solid '+bd,borderRadius:8,
    boxShadow:f?'0 0 0 3px rgba(26,86,219,.16)':'none',transition:'border-color 120ms, box-shadow 120ms',...style}},
    leadingIcon && React.createElement('span',{style:{color:t.fg3,display:'grid',placeItems:'center'}},leadingIcon),
    React.createElement('input',{onFocus:()=>setF(true),onBlur:()=>setF(false),
      style:{flex:1,minWidth:0,border:0,outline:0,background:'transparent',color:t.fg1,
        font:'400 '+(size==='small'?13:14)+'px/1 '+t.font},...rest}),
    trailingIcon && React.createElement('span',{style:{color:t.fg3,display:'grid',placeItems:'center'}},trailingIcon));
}