...
/**
 * The tones a name can hash onto, when a caller opts into hashing at all. These
 * are the four hue-named `accent.*` families the system ships — deliberately
 * the *hue* ramps and not the status ramps (`accent.critical`, `accent.success`,
 * …). A person is not an error, and an avatar tinted `accent-critical-*` reads
 * as one in every code review. `neutral` is not in this list on purpose: it is
 * the default every avatar starts from, not a hue a name can land on.
 */
export const AVATAR_TONES = ['blue', 'green', 'red', 'yellow'] as const;
export type AvatarTone = 'neutral' | (typeof AVATAR_TONES)[number];

/**
 * Deterministic name → tone, for a caller that explicitly wants to colour-code
 * people (e.g. distinguishing many strangers in a dense list). Not applied by
 * default — see `Avatar`'s own doc. The same string always lands on the same
 * colour, so a person keeps their tone across pages and reloads once a caller
 * opts in.
 *
 ...
 *
 * Three deviations from the export, all deliberate:
 *
 * 1. **Neutral by default, not colour-hashed.** The export tinted every
 *    initials avatar by hashing the name onto one of six pastels; Figma's own
 *    `Profile_Initials` draws plain `surface.neutral.subtle` at every size
 *    (design audit, 24 Sep 2026) — colour-hashing was never the default, it
 *    was the export inventing one. `toneForName` still exists for a caller
 *    that genuinely needs to tell people apart by colour; it just isn't
...
const avatar = cva(
  ...
  ],
  {
    variants: {
      ...
      tone: {
        /* surface.neutral + text.secondary — the default every avatar starts
           from, the same shape Badge's own neutral tone uses. */
    ...
    defaultVariants: { size: 'md', tone: 'neutral', ring: false },
  },
);
...
export interface AvatarProps
  ...
  /** The person's full name. Drives the initials. */
  name?: string;
  ...
  src?: string;
  /**
   * `neutral` — the default every avatar starts from. Pass `toneForName(name)`
   * to opt into colour-hashing instead, for contexts that need to tell many
   * people apart at a glance (a dense list of strangers); most call sites
...
export const Avatar = React.forwardRef<HTMLSpanElement, AvatarProps>(
  (
    ...
    return (
      <span
        ref={ref}
        className={cn(avatar({ size, tone, ring }), className)}
