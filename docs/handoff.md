...
**Pieces, as of 8 September:** 19 atoms · 31 components · 3 patterns ·
...
**1. One PR is open and green — [#82](https://github.com/VDM-Design-Team/VCP-design-system/pull/82).**
`Footer` and `PageTitle`, the two pieces `AppShell` needs. Nothing blocks it;
it wants a look and a merge.

It carries two questions for the design-system owner, both recorded because
guessing has cost a rebuild twice this week:
...
**2. Figma is behind the repo in six places — [issue #80](https://github.com/VDM-Design-Team/VCP-design-system/issues/80).**
...
**3. The plugin release still needs announcing by hand.** The self-update
...
build.** `TopBar`, `AVHeader` and `Sidebar` have all shipped; #82 adds the last
two pieces. The audit is already done, in `docs/figma-audit.md` batch 3 and in
#82's description:
